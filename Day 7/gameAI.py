    ...
    def heuristic(self, state: Board, ai_player: str) -> int:
        ...
        for line in self.model.lines:
            ...
            if opponent not in line_vals:
                score += 1
                if line_vals.count(ai_player) == 2:
                    score += 10  # Close to win

            # Negative score if line is open for Opponent
            if ai_player not in line_vals:
                score -= 1
                if line_vals.count(opponent) == 2:
                    score -= 10  # Block danger
...
class TicTacToeApp:
    def __init__(self, root):
        ...
        modes = ["BFS", "DFS", "A*"]
        for mode in modes:
            tk.Radiobutton(control_frame, text=mode, variable=self.algo_var, value=mode).pack(
                side=tk.LEFT
            )
        ...
        # Reset
        # FIX: 'mt' replaced with 'pady'
        tk.Button(self.main_frame, text="Reset Game", command=self.reset_game).grid(
    ...
    def update_gui(self):
        for i, val in enumerate(self.current_state):
            ...
            # Color logic
            if val == "X":
                self.buttons[i].config(disabledforeground="blue")
            if val == "O":
                self.buttons[i].config(disabledforeground="red")
