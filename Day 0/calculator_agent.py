...
button_add = tk.Button(
    root, text="+", command=lambda: calculate("+"), font=default_font, width=5, bg="red"
)
button_add.pack(pady=5)

button_sub = tk.Button(
    root, text="-", command=lambda: calculate("-"), font=default_font, width=5, bg="blue"
)
button_sub.pack(pady=5)

button_mul = tk.Button(
    root, text="*", command=lambda: calculate("*"), font=default_font, width=5, bg="yellow"
)
button_mul.pack(pady=5)

button_div = tk.Button(
    root, text="/", command=lambda: calculate("/"), font=default_font, width=5, bg="green"
)
