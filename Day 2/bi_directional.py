...
class Navigator:
    ...
    def solve_standard_bfs(self, start, end):
        ...
        while q:
            self.nodes_explored += 1
            u = q.popleft()
            if u == end: return self._reconstruct_path(visited, end)
...
def main():
    ...
    print(f"[Bi-Directional BFS]\t Nodes Explored: {nav.nodes_explored} \t(Path Length: {len(bi_bfs_path)})")

    # Bi-Directional DFS
    bi_dfs_path = nav.solve_bidirectional_dfs(start_node, end_node)
    print(f"[Bi-Directional DFS]\t Nodes Explored: {nav.nodes_explored} \t(Path Length: {len(bi_dfs_path)})")
